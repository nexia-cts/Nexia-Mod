...
public abstract class LivingEntityMixin {
    ...
    public void knockback(float f, double d, double e) {
        ...
        if (!(f <= 0.0F)) {
            ...
            instance.setDeltaMovement(vec3.x / 2.0 - vec32.x, instance.isOnGround() ? Mth.clamp((double)f * 0.75, 0.0, 0.4) : Mth.clamp(vec3.y + (double)f * 0.5, -0.24, 0.4), vec3.z / 2.0 - vec32.z);
        }
    }
