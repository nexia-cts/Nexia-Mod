    ...
    public void knockback(float f, double d, double e) {
        ...
        if (!itemStack.isEmpty()) {
            if(instance instanceof Player) {
                g = Math.min(2.0, 1-(1-g)*(1-(double) ShieldItem.getShieldKnockbackResistanceValue(itemStack)));
            } else {
                g = Math.min(2.0, g + (double) ShieldItem.getShieldKnockbackResistanceValue(itemStack));
            }
        }
        ...
        if (!(f <= 0.0F)) {
            ...
            instance.setDeltaMovement(vec3.x / 2.0 - vec32.x, instance.isOnGround() ? Mth.clamp((double)f * 0.75, 0.0, 0.4) : Mth.clamp(vec3.y + (double)f * 0.5, -0.32, 0.32), vec3.z / 2.0 - vec32.z);
        }
    }
