...
public class PlayerRespawnListener {
    public static void registerListener(){
        PlayerRespawnEvent.BACKEND.register((respawnEvent) -> {
            Player nexusPlayer = respawnEvent.getPlayer();
            if(nexusPlayer == null && respawnEvent.getEntity() instanceof Player) {
                nexusPlayer = (Player) respawnEvent.getEntity();
            }

            if(nexusPlayer == null) return;

            ServerPlayer player = ServerTime.minecraftServer.getPlayerList().getPlayer(nexusPlayer.getUUID());
            ...
            if(duelsGame != null && duelsGame.isEnding && duelsGame.winner != null) {
                nexusPlayer.getInventory().clear();
                LobbyUtil.giveItems(player);

                respawnEvent.setRespawnMode(Minecraft.GameMode.SPECTATOR);
                respawnEvent.setSpawnpoint(new Location(duelsGame.winner.get().getX(), duelsGame.winner.get().getY(), duelsGame.winner.get().getZ(), ServerTime.nexusServer.getWorld(new Identifier("duels", WorldUtil.getIdentifierWorldName(duelsGame.level).getId()))));
            } else if(teamDuelsGame != null && duelsData.duelOptions.duelsTeam != null) {
                nexusPlayer.getInventory().clear();
                LobbyUtil.giveItems(player);
                respawnEvent.setRespawnMode(Minecraft.GameMode.SPECTATOR);

                Location respawn = new Location(0,80, 0, ServerTime.nexusServer.getWorld(new Identifier("duels", WorldUtil.getIdentifierWorldName(teamDuelsGame.level).getId())));
                ...
                respawnEvent.setRespawnMode(Minecraft.GameMode.SPECTATOR);
                respawnEvent.setSpawnpoint(new Location(customDuelsGame.winner.get().getX(), customDuelsGame.winner.get().getY(), customDuelsGame.winner.get().getZ(), ServerTime.nexusServer.getWorld(new Identifier("duels", WorldUtil.getIdentifierWorldName(customDuelsGame.level).getId()))));
                //player.teleportTo(duelsGame.level, duelsGame.winner.getX(), duelsGame.winner.getY(), duelsGame.winner.getZ(), 0, 0);
                ...
                Location respawn = new Location(0,80, 0, ServerTime.nexusServer.getWorld(new Identifier("duels", WorldUtil.getIdentifierWorldName(customTeamDuelsGame.level).getId())));
