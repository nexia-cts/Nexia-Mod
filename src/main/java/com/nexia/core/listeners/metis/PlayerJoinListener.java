...
public class PlayerJoinListener {
    ...
    private static void checkBooster(ServerPlayer player) {
        ...
        if(discordUser == null) {
            if(Permissions.check(player, "nexia.prefix.supporter")) {
                if(Permissions.check(player, "nexia.rank")) {
                    ServerTime.metisServer.runCommand("/staffprefix set " + player.getScoreboardName() + " default");
                    ServerTime.metisServer.runCommand("/staffprefix remove " + player.getScoreboardName() + " supporter");
                    return;
                }
                ServerTime.metisServer.runCommand("/rank " + player.getScoreboardName() + " default", 4, false);
            }
            return;
        }
        ...
        if(hasRole && !hasSupporterPrefix) {
            if(Permissions.check(player, "nexia.rank")) {
                ServerTime.metisServer.runCommand("/staffprefix add " + player.getScoreboardName() + " supporter", 4, false);
                return;
            }
            ServerTime.metisServer.runCommand("/rank " + player.getScoreboardName() + " supporter", 4, false);
        } else if(!hasRole && hasSupporterPrefix) {
            if(Permissions.check(player, "nexia.rank")) {
                ServerTime.metisServer.runCommand("/staffprefix remove " + player.getScoreboardName() + " supporter", 4, false);
                ServerTime.metisServer.runCommand("/staffprefix set " + player.getScoreboardName() + " default", 4, false);
                return;
            }
            ServerTime.metisServer.runCommand("/rank " + player.getScoreboardName() + " default", 4, false);
        }
