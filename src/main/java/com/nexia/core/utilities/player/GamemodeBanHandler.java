...
public class GamemodeBanHandler {
    ...
    public static PlayerGameMode getBannedGameMode(NexiaPlayer player) {
        for(PlayerGameMode gameMode : PlayerGameMode.playerGameModes) {
            JSONObject banList = getBanList(player.player().uuid, gameMode);
    ...
    public static void addBanToList(NexiaPlayer player, PlayerGameMode gameMode, String reason, LocalDateTime duration) {
        try {
            ...
            jsonObject.put("uuid", player.player().uuid);
            ...
            Path path = new File(new File(dataDirectory), String.valueOf(player.player().uuid)).toPath();
            ...
            if(file.createNewFile()){
                FileWriter fileWriter = new FileWriter(dataDirectory + "/" +  player.player().uuid + "/" + gameMode.id + ".json");
    ...
    public static void tryGamemodeBan(CommandSourceStack sender, NexiaPlayer player, PlayerGameMode gameMode, int duration, String reason) {
        JSONObject banJSON = getBanList(player.player().uuid, gameMode);

        if (banJSON != null) {
            LocalDateTime banTime = getBanTime((String) banJSON.get("duration"));
            if(LocalDateTime.now().isAfter(banTime)) {
                removeBanFromList(player.player().uuid, gameMode);
            } else {
        ...
        sender.sendSuccess(LegacyChatFormat.format("{s}Gamemode ({}) banned {b2}{} {s}for {b2}{}{s}." +
                "\n{s}Reason: {b2}{}", gameMode.name, player.player().name, BanHandler.banTimeToText(banTime), reason), false);
    ...
    public static void tryUnGamemodeBan(CommandSourceStack sender, NexiaPlayer player, PlayerGameMode gameMode) {
        if (!removeBanFromList(player.player().uuid, gameMode)) {
            sender.sendSuccess(LegacyChatFormat.format("{s}This player is not banned."), false);
            return;
        }

        sender.sendSuccess(LegacyChatFormat.format("{s}Un-gamemode-banned {b2}{}{s} in {b2}{}{s}.", player.player().name, gameMode.name), false);
    }
}
