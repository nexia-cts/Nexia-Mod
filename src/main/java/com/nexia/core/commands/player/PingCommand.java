...
public class PingCommand {
    public static void register(CommandDispatcher<CommandSourceInfo> dispatcher) {
        dispatcher.register(CommandUtils.literal("ping").executes(PingCommand::run)
                .then(CommandUtils.argument("player", EntityArgument.player())
                        .executes(context -> PingCommand.ping(context, context.getArgument("player", ServerPlayer.class)))
                )
        );
    ...
    public static int run(CommandContext<CommandSourceInfo> context) throws CommandSyntaxException {
        ...
        int ping = PlayerUtil.getMinecraftPlayerFromName(((Player) context.getSource().getExecutingEntity()).getRawName()).latency;
